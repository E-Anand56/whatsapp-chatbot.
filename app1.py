...
# ---- Most Active Day ----
if data['Day'].dropna().empty:
    st.write("📅 **Most Active Day:** Data not available")
else:
    st.write(
        "📅 **Most Active Day:**",
        data['Day'].value_counts().idxmax()
    )

# ---- Most Active Hour ----
if data['Hour'].dropna().empty:
    st.write("🕒 **Most Active Hour:** Data not available")
else:
    st.write(
        "🕒 **Most Active Hour:**",
        f"{data['Hour'].value_counts().idxmax()}:00"
    )
